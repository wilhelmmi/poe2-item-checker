...
from app.schemas.management import (
    SLOTS, EquipmentExport, EquipmentImportData, EquipmentItem, EquipmentResponse,
    ProfileData, Slot,
)

BLOCKING = {"input_missing_line_breaks", "missing_item_identity", "no_modifiers_detected"}
SLOT_CLASSES = {
    "wand": "Wands", "focus": "Foci", "helmet": "Helmets",
    "body_armour": "Body Armours", "gloves": "Gloves", "boots": "Boots",
    "belt": "Belts", "ring_1": "Rings", "ring_2": "Rings", "amulet": "Amulets",
}
...
def get_or_create_profile(db: Session) -> CharacterProfile:
    ...
    if profile is None:
        profile = CharacterProfile(id=1, name="Chaos DoT Lich", build_stage="early_endgame", notes="")
        db.add(profile)
        db.flush()
    return profile
...
def profile_schema(profile: CharacterProfile) -> ProfileData:
    return ProfileData.model_validate({field: getattr(profile, field) for field in ProfileData.model_fields})
...
def parse_equipment(raw_text: str) -> ParsedItem:
...
def item_schema(item: Item) -> ParsedItem:
    values = {field: getattr(item, field) for field in ParsedItem.model_fields if field != "modifiers"}
    values["modifiers"] = [
        ModifierData.model_validate({field: getattr(modifier, field) for field in ModifierData.model_fields})
...
def replace_equipment(db: Session, slot: Slot, raw_text: str) -> EquipmentItem:
    parsed = parse_equipment(raw_text)
...
def import_equipment(db: Session, data: EquipmentImportData) -> EquipmentResponse:
    parsed = {slot: parse_equipment(raw) for slot, raw in data.equipment_raw_text.items() if raw is not None}
